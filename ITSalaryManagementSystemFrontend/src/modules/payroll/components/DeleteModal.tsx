...
export const DeleteModal = ({ data, successCallback }: Props) => {
  ...
  return (
    <>
      ...
      >
        <p>
          Are you sure to delete payroll in month "{dayjs(data.CreatedDate).format("MM/YYYY")}" of employee "
          {data.Contract.User.EmployeeCode} - {data.Contract.User.EmployeeName}"?
        </p>
      </Modal>
    </>
  );
