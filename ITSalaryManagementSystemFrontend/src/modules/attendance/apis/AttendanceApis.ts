...
const AttendanceApis = {
  getAll: (): Promise<Response<AttendanceModel[]>> =>
    AxiosClient.get("/Attendance?$expand=user"),
  post: (data: AttendanceReq): Promise<void> =>
    AxiosClient.post("/Attendance", data),
  ...
  patch: (id: number, attendanceStatus: number): Promise<void> => {
    return AxiosClient.patch(
      `/Attendance(${id})?attendanceStatus=${attendanceStatus}`
    );
  },
};
